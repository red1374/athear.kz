...
<? if ($arResult['ITEMS']) { ?>
    <section class="slider-block">
        <div class="_container">
            ...
            <? if ($arParams['SHOW_H1'] == 'Y'):?><h1><? else:?><h2><? endif?>
                <?=$arParams['TITLE'] ? $arParams['TITLE'] : 'Популярные слуховые аппараты'?>
            <? if ($arParams['SHOW_H1'] == 'Y'):?></h1><? else:?></h2><? endif?>
            <? if ($arParams['HIDE_BTN'] !== 'Y') { ?>
                <a href="/catalog/" class="btn btn--grey btn--m"><?=GetMessage('CT_SHOW_ALL')?></a>
            <?} ?>
            </div>
                            ...
                            <div class="card">
                                ...
                                <? if ($arParams['SHOW_BUY_BTN'] == 'Y') { ?>
                                    <button class="btn btn--red btn--icn btn--m"
                                            data-id="<?=$arItem['ID']?>" data-quantity="1" data-reload="true">
