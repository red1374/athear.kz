...
<section class="catalog">
    <div class="_container">
        <h1 class="title-page catalog__title"
            data-count="<? $APPLICATION->ShowProperty('element_count')?>"><? $APPLICATION->ShowTitle(false)?></h1>
        ...
        <?php
            if (preg_match('/^\/(?:.+?)filter\/(.+?)\/apply\//m', $_SERVER['REQUEST_URI'], $matches_f )){
            ...
            <div class="catalog__box">
                <div class="catalog__sort">
                    <a <?=$_COOKIE['sortten'] === 'show_counter' ? ' class="active"' : ' '?>
                            data-query="show_counter">Популярные</a>
                    <a <?=$_COOKIE['sortten'] === 'catalog_PRICE_1_max' ? ' class="active"' : ' '?>
                            data-query="catalog_PRICE_1_max">Дешевле</a>
                    <a <?=$_COOKIE['sortten'] === 'catalog_PRICE_1_min' ? ' class="active"' : ' '?>
                            data-query="catalog_PRICE_1_min">Дороже</a>
                    <!-- TODO сделать после создания свойства -->
                    <a <?=$_COOKIE['sortten'] === 'discount' ? ' class="active"' : ' '?>
                ...
                <div class="catalog__text text-block">
                    <h3 class="text-block__title"><? $APPLICATION->ShowTitle(false)?></h3>
