...
//			this.pickUpMap.behaviors.disable('scrollZoom');
		...
		buildBalloons: function(activeStores){
			...
			for (var i = 0; i < activeStores.length; i++){
                ...
                /*{ hintContent: BX.util.htmlspecialchars(activeStores[i].TITLE),
                ...
                },*/
                {
                    balloonContent:
                        ...
                        '<h3 class="popover-title">' + activeStores[i].TITLE + '</h3>' +
                        '<div class="popover-content">' + storeInfoHtml + '</div>'+
//						'<br /><a class="btn btn-sm btn-default" data-store="{{ properties.id }}">{{ properties.text }}</a>' +
                ...
                /* balloonOffset: [40, 40],
					balloonContentLayout: ymaps.templateLayoutFactory.createClass(
                        '<div class="popover top">' +
                        '<a class="close" href="javascript:void(0)" onclick="BX.Sale.OrderAjaxComponent.Maps.closeBaloon(this);return false;" data-close="Y"><img src="/local/templates/audiotech/images/icns/md-x.svg"></a>' +
                        '<div class="arrow"></div>' +
                        '<div class="popover-inner">' +
                        '<h3 class="popover-title">{{ properties.storeTitle }}</h3>' +
                        '<div class="popover-content">{{ properties.storeBody|raw }}</div>'+
//						'<br /><a class="btn btn-sm btn-default" data-store="{{ properties.id }}">{{ properties.text }}</a>' +
                        '</div>' +
                        '</div>'
...
//						'{{ properties.storeBody|raw }}' +
						...
						,{
							build: function() {
								...
								let button = document.querySelector('a[data-store]');
								if (button){
...
//				}
		...
		getStoreInfoHtml: function(currentStore){
			...
			if (currentStore.ADDRESS){
				html += `<li><span>${BX.message('SOA_PICKUP_ADDRESS')}</span><span>${BX.util.htmlspecialchars(currentStore.ADDRESS)}</span></li>`;
            }
			if (currentStore.SCHEDULE){
				html += `<li><span>${BX.message('SOA_PICKUP_WORK')}</span><span>${currentStore.SCHEDULE}</span></li>`;
            }
			if (currentStore.PHONE){
				html += `<li><span>${BX.message('SOA_PICKUP_PHONE')}</span><span>${BX.util.htmlspecialchars(currentStore.PHONE)}</span></li>`;
            }
			if (currentStore.EMAIL){
				html += `<li><span>${BX.message('SOA_PICKUP_DESC')}</span><span>${BX.util.htmlspecialchars(currentStore.DESCRIPTION)}</span></li>`;
            }
            html += `<li><span>${BX.message('SOA_PICKUP_PAYMENT')}</span><span>${BX.message('SOA_PICKUP_PAYMENT_VALUE')}</span></li>`;
            html += `<li><span class="popover__footer">${BX.message('SOA_PICKUP_DELIVERY')}</span><span>${BX.message('SOA_PICKUP_DELIVERY_VALUE')}</span></li>`;
