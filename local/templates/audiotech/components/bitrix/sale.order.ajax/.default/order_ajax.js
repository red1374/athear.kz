...
//					break;
		...
		locationsCompletion: function()
		{
			...
			for (i in this.locations)
			{
				...
				locationNode = this.orderBlockNode.querySelector('[data-property-id-row="' + i + '"]');
				if (!locationNode)
					continue;
		...
		fixLocationsStyle: function(section, hiddenSection)
		{
			...
			locationSearchInputs = regionActive.querySelectorAll('div.bx-sls div.order__city-wrap.bx-ui-sls-input-block');
			locationStepInputs = regionActive.querySelectorAll('div.bx-slst div.order__city-wrap.bx-ui-slst-input-block');
		...
		getBlockFooter: function(node)
		{
				...
				buttons.push(
					BX.create('A', {
						props: {
							...
							className: 'pull-left btn btn--grey btn--m'
						},
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: 'javascript:void(0)', className: 'pull-right btn btn--red btn--m'},
		...
		editBasketItemsHeader: function(basketItemsNode)
		{
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++){
				...
				headers.push(
					BX.create('DIV', {
						props: {className: 'table__item' + (toRight ? ' bx-text-right' : '')},
						text: column.name,
					})
				);
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'table__row table__header'},
					children: headers
				})
			);
		},
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'table__row' + (index == 0 ? ' bx-soa-item-tr-first' : '')},
					children: cols
				})
			);
		},
		...
		createBasketItemColumn: function(column, allData, active){
			...
			if (column.id === 'PRICE_FORMATED'){
                ...
                inner_text = data.PRICE_FORMATED;
				if (parseFloat(data.DISCOUNT_PRICE) > 0)
				{
                    ...
                    inner_text += ` ${data.BASE_PRICE_FORMATED}`;
				}
			...
			else if (column.id === 'SUM')
			{
				...
				if (parseFloat(data.DISCOUNT_PRICE) > 0)
				{
                    ...
                    inner_text = data.SUM_BASE_FORMATED;
				}
                inner_text = data.SUM;
			}
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				regionNodeCol = BX.create('DIV', {props: {className: 'profile__list'}});
		...
		editFadeRegionContent: function(node)
		{
			...
			if (locationProperty && locationString.length)
				addedHtml += `<p>${BX.util.htmlspecialchars(locationString)}</p>`;
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output){
				...
				labelHtml = '<label class="label" for="soa-property-' + parseInt(locationId) + '">'
				...
				insertedLoc = BX.create('DIV', {
					...
					props: {className: 'form-group profile__item'},
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty){
				...
				propsItemNode = BX.create('DIV', {props: {className: "form-group profile__item"}});
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-7 bx-soa-pp-item-container order__methods'}}),
				paySystemItemNode, i;
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				title, label, itemNode, radio, input;
            ...
            input = BX.create('INPUT', {
                props: {
                    ...
                    name: 'PAY_SYSTEM_ID',
                    type: 'checkbox',
                    className: 'bx-soa-pp-company-checkbox radio',
                    value: paySystemId,
                    checked: checked
                }
            });
			...
			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y'){
				title = BX.create('SPAN', {props: {className: 'bx-soa-pp-company-smalltitle method__name'}, text: item.NAME});
			}

            radio = BX.create('SPAN', {
                props: {className: 'method__radio'},
            });

			itemNode = BX.create('LABEL', {
				props: {className: 'bx-soa-pp-company order__method method'},
				children: [input, title, radio],
				events: {
			...
			if (checked)
				BX.addClass(itemNode, 'bx-selected active');
		...
		editFadePaySystemContent: function(node){
			...
			if (this.isSelectedInnerPayment())
			{
				...
				addedHtml += '<span>' + this.result.INNER_PAY_SYSTEM.NAME + '</span>';
				//addedHtml += '</div>';
			}
			...
			if (selectedPaySystem && selectedPaySystem.NAME)
			{
				...
				//addedHtml += '<div class="bx-soa-pp-company-selected">';
				//addedHtml += '<img src="' + imgSrc + '" style="height:18px;" alt="">';
				addedHtml += '<span>' + BX.util.htmlspecialchars(selectedPaySystem.NAME) + '</span>';
				//addedHtml += '</div>';
			}
		...
		editActiveDeliveryBlock: function(activeNodeMode){
			var node = activeNodeMode ? this.deliveryBlockNode : this.deliveryHiddenBlockNode,
				deliveryContent, deliveryNode, currentDelivery, deliveryID;
			...
			if (this.initialized.delivery){
                ...
                if (!currentDelivery){
                    if (deliveryID = document.querySelector('[name=DELIVERY_ID]').value){
                        currentDelivery = this.getDelivetyById(deliveryID);
                    }
                }
                if (currentDelivery.STORE.length){
                    this.editDeliveryPickUpInfo(deliveryNode);
                }
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-7 bx-soa-pp-item-container order__methods'}}),
				deliveryItemNode, k;
		...
		editDeliveryPickUpInfo: function(deliveryNode){
			if (!this.result.DELIVERY)
				return;

			let deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-5 bx-soa-pp-desc-container'}}),
				currentDelivery, store;

			BX.cleanNode(deliveryInfoContainer);
			currentDelivery = this.getSelectedDelivery();
            store = this.getPickUpInfoArray([currentDelivery.STORE[0]])[0];

            if (store.ADDRESS){
                deliveryInfoContainer.appendChild(BX.create('P', {
                    html: `<strong>${BX.message('SOA_PICKUP_ADDRESS')}:</strong> ` + store.ADDRESS,
                }));
            }

            if (store.GPS_N && store.GPS_S){
                deliveryInfoContainer.appendChild(BX.create('P', {children : [
                    BX.create('A', {
                        text: BX.message('SOA_PICKUP_ON_THE_MAP'),
                        props: {
                            href: 'javascript:void(0)',
                            className: 'link',
                        },
                        events: {
                            click: BX.proxy(this.showPickUpMap, this),
                        },
                    }),
                ]}));
        ...
        showPickUpMap: function(){
            let mapBlock = BX('pickUpMap');

            if (mapBlock){
                BX.removeClass(mapBlock, 'dev-hide');
            }
		...
		createDeliveryItem: function(item){
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode, logoNode, radio;
			...
			if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y'){
				title = BX.create('SPAN', {
					props: {className: 'bx-soa-pp-company-smalltitle method__name'},
            ...
            labelNodes.push(radio);

			itemNode = BX.create('LABEL', {
				props: {className: 'bx-soa-pp-company order__method method'},
				children: labelNodes,//[label, title, radio],
				events: {click: BX.proxy(this.selectDelivery, this)}
			});
			checked && BX.addClass(itemNode, 'bx-selected active');
        ...
        getPricePrint(price){
            if (!price.indexOf('0 ')){
                return BX.message('SOA_FREE_TITLE');
            }else{
                return price;
            }
        },
		...
		editFadeDeliveryContent: function(node){
			...
			if (selectedDelivery && selectedDelivery.NAME)
			{
                ...
                arNodes.push(this.getPricePrint(this.getDeliveryPriceNodes(selectedDelivery)[0]));
                let p = BX.create('P', {
						props: {
		...
		getSelectedDelivery: function(){
			...
			if (deliveryCheckbox){
                currentDelivery = this.getDelivetyById(deliveryCheckbox.value);
			}

			return currentDelivery;
		},

        getDelivetyById(deliveryId){
		...
		activatePickUp: function(deliveryName){
			...
			this.pickUpBlockNode.querySelector('.order__title').innerHTML =
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('UL', {props: {className: 'bx-soa-customer profile__list form__wrapper'}}),
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled){
			var propsItemNode = BX.create('LI'),
				textHtml = '',
				propertyType = property.getType() || '',
				propertyDesc = property.getDescription() || '',
				label, blankLi, arBlankFiels = ['SECONDNAME', 'AGREE1', 'AGREE2'];

			if (disabled){
				propsItemNode.innerHTML = '<strong>' + BX.util.htmlspecialchars(property.getName()) + ':</strong> ';
			}else{
				BX.addClass(propsItemNode, "form-group bx-soa-customer-field profile__item");
				...
				if (propertyDesc.length && propertyType != 'STRING' && propertyType != 'NUMBER' && propertyType != 'DATE'){
                    if (propertyType != 'Y/N'){
                        ...
                        textHtml = `<span>${propertyDesc}</span>`;
                    }
                }

				label = BX.create('LABEL', {
					attrs: {'for': 'soa-property-' + property.getId()},
					props: {className: 'bx-soa-custom-label label'},
					html: textHtml
				});
			...
			switch (propertyType){
				...
				case 'Y/N':
                    ...
                    propsItemNode.querySelector('DIV').appendChild(label);
					break;
				case 'NUMBER':
					this.insertNumberProperty(property, propsItemNode, disabled);
			}

            if (property.getSettings()['CODE'] == 'STREET'){
                propsItemsContainer.appendChild(BX.create('LI', {
                    props : {className: 'propsprofile__item'},
                    children: [BX.create('h4', {text: BX.message('SOA_ADDRESS_BLOCK')})],
                }));
                propsItemsContainer.appendChild(BX.create('LI', {
                    props : {className: 'propsprofile__item profile__item--empty'}}));
            }

			propsItemsContainer.appendChild(propsItemNode);

            if (arBlankFiels.includes(property.getSettings()['CODE'])){
                propsItemsContainer.appendChild(BX.create('LI', {props : {className: 'propsprofile__item profile__item--empty'}}));
            }
		},
		...
		insertStringProperty: function(property, propsItemNode, disabled)
		{
			...
			if (disabled){
				prop = this.propsHiddenBlockNode.querySelector('[data-property-id-row="' + property.getId() + '"]');
				if (prop){
					values = [];
		...
		insertYNProperty: function(property, propsItemNode, disabled){
			...
			if (disabled){
				...
				propContainer = BX.create('DIV', {
                    props: {className: 'soa-property-container modal__check order__check'}
                });
				property.appendTo(propContainer);
				propsItemNode.appendChild(propContainer);
				this.alterProperty(property.getSettings(), propContainer);
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++){
				BX.addClass(inputs[i], 'input-action');
                inputs[i].setAttribute('id', 'soa-property-' + settings.ID);
            }

			inputs = propContainer.querySelectorAll('textarea');
			for (i = 0; i < inputs.length; i++){
				inputs[i].placeholder = settings.DESCRIPTION;
				BX.addClass(inputs[i], 'form-control bx-ios-fix order__message');
			}
		...
		showValidationResult: function(inputs, errors){
            ...
            let input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
                formGroup = BX.findParent(input0, {tagName: 'LI', className: 'form-group'});
            formGroup = formGroup ? formGroup : BX.findParent(input0, {tagName: 'DIV', className: 'form-group'});
			...
			for (i = 0; i < inputs.length; i++){
                if (errors[i] && errors[i].length){
                    BX.addClass(inputs[i], 'error_input');
                }
				/*inputDiv = BX.findParent(inputs[i], {tagName: 'DIV', className: 'form-group'});
				if (errors[i] && errors[i].length)
					BX.addClass(inputDiv, 'error_input');
				else
					BX.removeClass(inputDiv, 'error_input');*/
			}
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'order__block mt-40'}});
			/*label = BX.create('LABEL', {
			...
			input = BX.create('TEXTAREA', {
				props: {
					id: 'orderDescription',
					cols: '4',
					className: 'form-control bx-soa-customer-textarea bx-ios-fix input order__message',
		...
		editTotalBlock: function()
		{
			...
			if (!this.result.SHOW_AUTH){
                /*let button = document.querySelector('.btn-order-save');
                if (button){
                ...
                let old_btn = document.querySelector('.bx-soa-cart-total-button-container');
				//this.totalInfoBlockNode
                if (old_btn){
                    old_btn.remove();
                }
                this.totalBlockNode.appendChild(
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
									className: 'btn btn--red btn--icn btn--l order-btn btn-order-save'
								},
								html: `<svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    </svg><span>${this.params.MESS_ORDER}</span>`,
								events: {
		...
		createTotalUnit: function(name, value, params){
			var totalValue, className = 'bx-soa-cart-total-line aside-box__item',
                    span1Class = '', span2Class = '';
			...
			return BX.create('LI', {
				...
				children: [
					BX.create('SPAN', {props: {className: 'bx-soa-cart-t aside-box__text' + span1Class}, text: name}),
					BX.create('SPAN', {
						props: {
							className: 'bx-soa-cart-d aside-box__text' + span2Class +
